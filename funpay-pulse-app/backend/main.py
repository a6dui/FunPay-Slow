...
# --- ENDPOINTS ---
@app.get("/")
def root(): return {"status": "ok", "version": "2.2.2"}
...
def post_support(msg: SupportMessage):
    tg_text = f"📩 <b>Поддержка</b>\n👤 {msg.username} (ID: {msg.user_id})\n📝 Тип: {msg.type}\n💬 {msg.message}"
...
def activate_trial(req: TrialRequest):
    ...
    c = conn.cursor()
    ph = get_ph()
    c.execute(f"SELECT trial_used FROM subscriptions WHERE user_id = {ph}", (str(req.user_id),))
    row = c.fetchone()
    if row and row[0] == 1:
        conn.close()
        return {"success": False, "message": "Пробный период уже использован."}
    
    expires = (datetime.now() + timedelta(days=4)).strftime("%Y-%m-%d %H:%M:%S")
    if row:
        c.execute(f"UPDATE subscriptions SET plan='Fast', expires_at={ph}, status='active', trial_used=1 WHERE user_id={ph}", (expires, str(req.user_id)))
    else:
        c.execute(f"INSERT INTO subscriptions (user_id, plan, expires_at, status, trial_used) VALUES ({ph}, 'Fast', {ph}, 'active', 1)", (str(req.user_id), expires))
    
    conn.commit()
    conn.close()
    send_admin_tg(f"🎁 Пробный период: {req.user_id}")
